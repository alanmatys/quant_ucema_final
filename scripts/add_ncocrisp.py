"""Backtest CRISP and splice it into the existing result files.

NCO_CRISP (Nested Clustered Optimization with CRISP-regularised
allocation) is added as a 25th comparator. Rather than re-run every strategy,
we backtest only CRISP for the headline, post-COVID and weekly windows,
append it to the metric CSVs and the daily-return pickles, and re-run
the inference (Ledoit-Wolf, Hansen SPA, bootstrap CIs) on the augmented
candidate set.
"""
...
from src.portfolio_maker import HRP, NCOCrisp
