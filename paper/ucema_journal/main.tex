...
paper, Marchenko--Pastur denoising and spectral detoning, and broaden
the comparison to 23 constructed portfolio strategies: thirteen
HRP-family variants (the baseline, eight correlation/covariance
constructions, and four embedding-based variants that learn the
clustering distance from path signatures, a graph embedding and two
contrastive sequence encoders); four risk-based comparators (IVP, MVP,
ERC, Maximum Diversification); three nested-clustering optimisers (HERC,
NCO and a return-tilted NCO); and three crypto-momentum strategies.
Every comparison runs on a survivorship-bias-corrected, point-in-time
Binance universe (146 ever-included symbols, delisted assets such as
LUNA and FTT included) and is wrapped in the reference Hansen Superior
Predictive Ability test and a studentized Ledoit--Wolf Sharpe-difference
test.
On 76 monthly rebalances over 2020-02 to 2026-05 the thirteen HRP-family
variants lie within a narrow annualised-Sharpe band (0.70--0.75): across
an exhaustive 547-cell hyperparameter grid, no correlation, distance,
linkage or embedding choice separates from baseline HRP (combined Hansen
SPA $p_{\text{consistent}}=0.91$). The one exception is \emph{Nested
Clustered Optimization}, which re-optimises the allocation within and
across clusters rather than only re-drawing the dendrogram: NCO reaches
Sharpe 0.93, a $+0.19$ edge over HRP that also holds in the post-COVID
sub-sample, though even this is not statistically significant
(Ledoit--Wolf $p=0.22$; headline SPA $p_{\text{consistent}}=0.41$).
A power analysis explains the gap: on this 6.3-year sample a
structurally distinct strategy must beat HRP by $\approx 0.4$--$0.8$
annualised Sharpe to be detected at 80\% power, so NCO's positive but
statistically unresolved edge remains uncertified. Tilting NCO toward an
estimated-return objective instead destroys it ($-97\%$ total return),
an illustration of the estimation-error amplification that motivates
risk-based allocation. A passive 100\% BTC hold (Sharpe 0.87) still
beats every diversified strategy except MVP and NCO, both of which win
by concentrating in BTC; in the post-COVID regime every diversified HRP
variant loses 51--64\% of capital while BTC gains. We retract an earlier
cluster-stability claim that a 76-snapshot re-analysis falsifies
(Pearson ARI wins 69\% of month-pairs). The paper maps which HRP design
choices affect performance and which do not: the allocation step
matters, the clustering step does not.
\end{abstract}
...
\item \textbf{HRP variants, embedding variants, and nested-clustering
      ...
      tail-dep$+$LW-shrunk hybrid) and \emph{four embedding-based
      ...
      We also add three strategies that change the \emph{allocation} step
      rather than the dendrogram: Hierarchical Equal Risk Contribution
      \citep{raffinot2018herc}, Nested Clustered Optimization
      \citep{lopezdeprado2019nco}, and a return-tilted NCO
      (\S\ref{sec:methods-comparators}); these test whether the clustering
      step or the allocation step drives performance. ERC
...
\item \textbf{Empirical characterisation and a retraction.}
...
worse.} Under the studentized Ledoit--Wolf test, no strategy is
significantly \emph{better} than baseline HRP, including NCO ($+0.19$
Sharpe, $p=0.22$) and MVP ($+0.31$, $p=0.06$). The only result that
crosses $p<0.05$ is the return-tilted NCO, significantly \emph{worse}
($-0.74$ Sharpe, $p=0.009$); ERC sits on the threshold ($-0.078$,
$p=0.051$). With the bug-fixed HRP\_PartialCorr ($\alpha=10^{-3}$,
\S\ref{sec:methods-cov}) the strategy is now distinct from IVP; it was
byte-identical to IVP in an earlier draft (Sharpe 0.697), and corrected
it sits at 0.720.
...
contender.} The three nested-clustering optimisers separate clearly.
HERC (0.712) sits \emph{inside} the HRP-family band; swapping recursive
bisection for an explicit cluster cut plus an equal-risk-contribution
allocation changes little. NCO is different: at Sharpe \textbf{0.933} it
is the second-best strategy in the table, a $+0.19$ edge over baseline
HRP and far outside the HRP-family band. The mechanism is informative.
NCO's nested minimum-variance steps concentrate weight toward
low-volatility assets (average maximum weight 0.42, effective
$N\approx5$ against HRP's $\approx30$), and in a BTC-led market that
means tilting into BTC; NCO is, in effect, a denoised, less-extreme
cousin of MVP. The return-tilted NCO is the cautionary opposite:
replacing the minimum-variance steps with momentum-driven maximum-Sharpe
steps turns it into a 164\%-turnover momentum-chaser that loses 97\% of
capital, an illustration of the estimation-error amplification that
risk-based allocation is designed to avoid. Within this dataset,
horizon and search space, changing the clustering step produced no
detectable outperformance over baseline HRP, whereas changing the
allocation step changed performance materially, both upward (NCO) and
downward (return-tilted NCO).
...
family, with Sharpes $0.703$--$0.714$, all marginally below baseline HRP
($-0.027$ to $-0.038$) and none significantly different from it
(Ledoit--Wolf $p \in [0.10, 0.27]$). Embedding methods do not deliver an
out-of-sample Sharpe edge on this universe and horizon; we treat this as
a null result. Section~\ref{sec:results-sweeps} tests the conclusion
...
are all the same bet at bottom: concentrate in BTC. MVP does it most
...
($[0.08, 1.63]$) have intervals entirely above zero. Baseline HRP's
...
$p=0.051$) sits on the threshold. The largest edge in the table,
NCO's $+0.19$ Sharpe, is statistically indistinguishable from noise.
...
substantively from HRP (NCO, MVP, the momentum family, HODL\_BTC) the
...
having searched the entire grid, yields $p_{\text{consistent}}=0.91$
($p_{\text{lower}}=0.73$, $p_{\text{upper}}=0.91$). Although 38 of 547
cells show a nominally positive Sharpe difference, the best of them
(HRP\_ShrunkCov, $\Delta=+0.024$) carries a pairwise Ledoit--Wolf $p$ of
$0.16$. \textbf{After correcting for the multiple testing of 547
...
low-volatility assets through nested minimum variance, it helps in
...
matters}
The central comparison in this paper is between two design choices. We
searched the \emph{clustering} step exhaustively (three correlation
estimators, four distances, four linkages, four embedding families,
multi-channel inputs, 547 hyperparameter cells), and no configuration
escaped the HRP-family Sharpe band; the combined Hansen SPA gives
$p_{\text{consistent}}=0.91$. We then changed the \emph{allocation} step
in three strategies, and performance moved in both directions: NCO
(nested minimum variance) to Sharpe $0.93$, HERC (equal-risk-contribution
across clusters) to $0.71$, and the return-tilted NCO to $-97\%$. HRP
and all its variants share one allocation rule, inverse-variance
recursive bisection, and that rule, not the dendrogram, is what holds
the family together. For practitioners the implication is narrow: on
the evidence in this study, additional effort on the clustering step
appears lower-yield than effort on the allocation step.
...
strategy whose allocation departs substantively from HRP must beat it by
$\approx0.4$--$0.8$ annualised Sharpe to be detected at 80\% power;
NCO's $+0.19$, and even MVP's $+0.31$, fall below that threshold. The
reading of the null is therefore precise: no strategy beats HRP by an
edge this design can resolve, which, given the power floor, leaves
NCO's positive observed edge an open question rather than a refuted one.

\subsection{Detoning hurts in BTC-led regimes}
Removing the dominant market mode is theoretically elegant, in that it
isolates idiosyncratic structure, but in our window the dominant mode
\emph{was} BTC, and being underweight BTC was costly. Detoning (and
partial correlation) may yet be valuable in the sideways or rotational
regimes 2020--2026 did not contain: their value is regime-dependent
rather than a universal Sharpe improvement.
...
strategies above it, MVP and NCO, get there by concentrating back into
BTC. Over 2020--2026, diversification in cryptocurrency paid a cost in
foregone BTC upside that the lower drawdowns of diversified portfolios
did not recover. This is a statement about the regime, a single asset
dominating a young and highly correlated market, rather than a flaw in
HRP. The post-COVID cut is the counterweight: there, diversified HRP
variants lose 51--64\% while concentration still wins. The lesson is
about regime dependence: in this asset class and over this period,
realised returns have been dominated by the direction of BTC.
...
We evaluated 23 constructed portfolio strategies (thirteen HRP-family
variants: the baseline, eight correlation/covariance constructions and
four embedding-based variants; four risk-based comparators; three
...
The central finding is a contrast between two design choices. Varying
the \emph{clustering} step, that is the correlation, distance, linkage
or learned embedding that builds the dendrogram, did not produce a
detectable outperformance over baseline HRP: across a 547-cell
hyperparameter grid no configuration escapes the HRP-family Sharpe
band, and the combined Hansen SPA cannot reject the null
...
collapses to $-97\%$. Even NCO's edge is not statistically significant:
a power analysis shows the design can only resolve Sharpe differences
above $\approx0.4$ for strategies that depart substantively from HRP,
so the null is a statement about detectable effect size rather than a
...
beat \emph{it} (MVP, NCO) win by concentrating back into BTC; in this
asset class and regime, portfolio construction has been second-order to
the BTC beta. Second, in the post-COVID sub-sample every diversified
HRP variant lost 51--64\% of capital, so the ``diversification works''
reading of the full window is itself regime-conditional. We also
retract an earlier cluster-stability claim that the 76-snapshot
re-analysis falsifies.
...
practitioner can vary, only the allocation objective produces a
detectable change, and even the one strategy that improves on baseline
(NCO) does so by an edge this sample size cannot certify.
...
HRP in choppier regimes. Both signals are too weak to confirm here but
