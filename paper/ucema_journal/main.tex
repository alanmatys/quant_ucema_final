...
the comparison to 27 constructed portfolio strategies: thirteen
...
contrastive sequence encoders); five risk-based comparators (IVP, MVP,
ERC, Maximum Diversification, and the CRISP correlation-shrinkage
portfolio); four nested-clustering optimisers (HERC, NCO, a
return-tilted NCO, and an NCO--CRISP hybrid); three crypto-momentum
strategies; and two signal-aware extensions (NCOML, an NCO fed by a
walk-forward XGBoost return forecast, and Wuebben's HRP-$\Sigma\mu$
hierarchical optimiser fed by the same forecast). Every comparison runs on a survivorship-bias-corrected,
point-in-time Binance universe (146 ever-included symbols, delisted
assets such as LUNA and FTT included) and is wrapped in the reference
Hansen Superior Predictive Ability test and a studentized Ledoit--Wolf
...
SPA $p_{\text{consistent}}=0.91$). Performance moves only when the
\emph{allocation} step changes. Four risk-based allocators reach Sharpe
0.93--1.06 (MVP, the CRISP correlation-shrinkage portfolio, an
NCO--CRISP hybrid, and NCO), and CRISP and NCO--CRISP beat baseline HRP
at the pairwise level in both the full and post-COVID windows
(Ledoit--Wolf $p\approx0.01$). But the Hansen SPA, correcting for the
27-candidate search, does not reject the null
($p_{\text{consistent}}=0.51$): no edge survives the multiple-testing
correction; the Deflated Sharpe Ratio, a second selection-aware test,
concurs. A power analysis explains the gap: the detectable Sharpe
edge scales with concentration, and these allocators' edges sit close to
their detection thresholds. Tilting an allocator toward a sample-mean
estimated-return objective destroys it (return-tilted NCO, $-97\%$ total
return); replacing that signal with a walk-forward XGBoost forecast
sidesteps the collapse (NCOML, Sharpe $0.80$) but does not outperform
signal-blind NCO, while the same forecast fed into Wuebben's HRP-$\Sigma\mu$
hierarchical optimiser underperforms baseline HRP (Sharpe $0.61$).
Together these localise the NCO\_RT collapse to signal quality, not to
the principle of return-aware allocation; they also show that adding a
credible learned signal does not break the multiple-testing-corrected null. A passive 100\% BTC
hold (Sharpe 0.87) still beats every diversified strategy except the
BTC-concentrating risk allocators; in the post-COVID regime every
...
re-analysis falsifies (Pearson ARI wins 69\% of month-pairs). The paper
maps which HRP design choices affect performance and which do not: the
allocation step matters, the clustering step does not.
\end{abstract}
...
\item \textbf{HRP variants, embedding variants, and nested-clustering
      ...
      clustering step or the allocation step drives performance. ERC
...
\paragraph{Pairwise significance.} Under the studentized Ledoit--Wolf
test, two strategies beat baseline HRP at $p<0.05$: the CRISP
correlation-shrinkage portfolio ($+0.25$ Sharpe, $p=0.012$) and the
NCO--CRISP hybrid ($+0.24$, $p=0.011$). MVP's larger raw edge ($+0.31$)
does not cross the threshold ($p=0.059$); its heavier concentration
makes the paired Sharpe difference noisier. NCO ($+0.19$, $p=0.22$) is
well short. In the other direction the return-tilted NCO is
significantly \emph{worse} ($-0.74$ Sharpe, $p=0.009$) and ERC sits on
the threshold ($-0.078$, $p=0.051$). These pairwise tests do not correct
for the 27-candidate search; the Hansen SPA, which does, is reported in
\S\ref{sec:results-inference} and does not reject. With the bug-fixed
HRP\_PartialCorr ($\alpha=10^{-3}$, \S\ref{sec:methods-cov}) the strategy
is now distinct from IVP; it was byte-identical to IVP in an earlier
draft (Sharpe 0.697), and corrected it sits at 0.720.
...
measurable on top of it. The two signal-aware extensions further localise
the NCO\_RT failure. NCOML, which keeps NCO's nesting but replaces the
sample-mean tilt by the walk-forward XGBoost forecast of
\S\ref{sec:methods-comparators}, reaches Sharpe $0.804$: it sidesteps
the collapse and edges above baseline HRP, but lands below pure
(signal-blind) NCO, so the learned signal does not add value over
NCO's risk-only allocation. HRP-$\Sigma\mu$, the same forecast fed into
Wuebben's signal-aware hierarchical optimiser, lands at $0.607$, below
baseline HRP, a direct demonstration that forcing a return signal into
HRP's clustering hurts in this market, where the dendrogram is robust
precisely because it is signal-blind. Changing the clustering step
produced no detectable outperformance over baseline HRP; changing the
allocation step moved performance materially, in both directions.
...
\textbf{The pairwise null is therefore a statement about detectable
effect size given concentration; it is the multiple-testing-corrected
SPA, not the per-strategy power, that ultimately declines to reject.}
...
post-COVID window two of the four edge clearly \emph{above} baseline
HRP (\texttt{HRP\_Contrastive} $+0.008$, \texttt{HRP\_PathSig}
$+0.007$), \texttt{HRP\_TS2Vec} essentially ties HRP ($-0.001$), and
\texttt{HRP\_NodeEmbed} ($-0.016$) stays below. None reaches individual significance (all
...
live-market evidence is the sharp opposite: a momentum-tilted NCO is
...
the $-97\%$ collapse and confirming the failure was signal quality
...
\emph{below} baseline HRP: forcing a return signal into HRP's clustering
hurts in this market, and the HRP family's strength is therefore in
...
\item PIT universe is ranked by Binance quote volume, not market cap.
    ...
    robustness check.
\item Cross-platform reproducibility of the trained models. When the
    full pipeline was re-run on independent hardware, every strategy
    reproduced bit-identically except the two PyTorch-trained encoders
    (HRP\textsubscript{Contrastive}, HRP\textsubscript{TS2Vec}); the
    XGBoost forecast that drives NCOML and HRP-$\Sigma\mu$ falls into
    the same category. Trained-model fits are only conditionally
    reproducible: multi-threaded BLAS reductions and platform-specific
    kernels make them bit-identical only within a fixed software stack.
    We pin the encoders with single-threaded execution and
    \texttt{torch.use\_deterministic\_algorithms}, and the XGBoost
    pipeline with \texttt{tree\_method='hist'}, \texttt{n\_jobs=1} and
    a fixed seed under single-threaded BLAS; the reported figures
    (generated on macOS~26, Apple Silicon, with Python~3.11,
    PyTorch~2.12 and XGBoost~3.2) are verified by
    \texttt{scripts/verify\_embedding\_determinism.py} and
    \texttt{scripts/verify\_signal\_determinism.py}, both of which
    confirm bit-identical run-to-run output. On a different operating
    system or processor architecture the BLAS kernels differ and the
    residual floating-point noise can propagate through the discrete
    single-linkage step and the XGBoost tree splits, moving the
    affected strategies' Sharpes by up to $\approx 0.02$ and the
    Ledoit--Wolf $p$-values attached to them correspondingly; the
    headline ranking is unaffected.
\end{enumerate}
...
We evaluated 27 constructed portfolio strategies (thirteen HRP-family
variants: the baseline, eight correlation/covariance constructions and
four embedding-based variants; five risk-based comparators; four
nested-clustering optimisers; three momentum strategies; and two
signal-aware extensions that pair existing nested/hierarchical logic
with a walk-forward XGBoost return forecast) on a
...
that two of the four embedding variants edge \emph{ahead} of baseline
HRP in choppier regimes (with a third essentially tied). Both signals are too weak to confirm here but
