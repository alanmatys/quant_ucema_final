...
variant decisively above baseline HRP. Phase~8 addresses this with an
...
channels do carry usable structure; but no multi-channel configuration
clears baseline HRP, and adding the fourth through sixth channels
plateaus or mildly regresses performance --- a bias--variance ceiling
against only $\sim$76 rebalance dates.
...
(53 monthly rebalances over $\sim$4.4 years). All other parameters
identical.

\textbf{The picture flips entirely.} On post-COVID data, baseline
HRP achieves Sharpe $0.047$ with total return $-60\%$ --- a
near-zero risk-adjusted return achieved by losing 60\% of capital
in straightforward dollar terms. All 19 strategies except MVP
(Sharpe $0.482$, $+66\%$) lose money in total terms over the
post-COVID window. Five strategies have outright \emph{negative}
Sharpe (vs zero in the full-window analysis): MaxDiv ($-0.163$),
ERC ($-0.026$), CS\_MOM ($-0.161$), RM\_MOM ($-0.160$, fixed
variant), and HODL\_BTC ($-0.492$, dragged by 2022 drawdown).
...
\textbf{(2) Embedding variants directionally lead baseline HRP in
the harder regime.} In the full window, all four embedding variants
were marginally below HRP (deltas $-0.018$ to $-0.042$ Sharpe). In
post-COVID, all four are \emph{above} HRP (\texttt{HRP\_Contrastive}
$+0.024$, \texttt{HRP\_NodeEmbed} $+0.011$, \texttt{HRP\_PathSig}
$+0.008$, \texttt{HRP\_TS2Vec} $+0.004$). None reach individual
significance (all LW $p > 0.5$), but the directional reversal is
suggestive: embedding methods may capture risk structure more
relevant in challenging regimes than in monotone rallies. This
would warrant follow-up with a longer post-COVID sample.
...
\midrule
HRP                 &  0.741 &  0.047 & --0.694\\
HRP\_Dynamic\_94    &  0.749 &  0.099 & --0.650\\
HRP\_ShrunkCov      &  0.746 &  0.038 & --0.708\\
HRP\_Contrastive    &  0.723 &  0.071 & --0.652\\
HRP\_NodeEmbed      &  0.721 &  0.057 & --0.664\\
HRP\_PathSig        &  0.714 &  0.055 & --0.659\\
HRP\_TS2Vec         &  0.699 &  0.051 & --0.648\\
MVP                 &  1.057 &  0.482 & --0.575\\
MaxDiv              &  0.719 & --0.163 & --0.882\\
ERC                 &  0.663 & --0.026 & --0.689\\
CS\_MOM\_eq21       &  0.663 & --0.161 & --0.824\\
RM\_MOM             &  0.663 & --0.160 & --0.823\\
HODL\_BTC           &  0.519 & --0.492 & --1.011\\
\bottomrule
\end{tabular}
\end{table}
