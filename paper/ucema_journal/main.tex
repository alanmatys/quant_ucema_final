...
broaden the comparison to 23 constructed portfolio strategies: thirteen HRP-family
variants (the baseline; eight correlation/covariance constructions; and
...
On 76 monthly rebalances over 2020-02 to 2026-05 the thirteen HRP-family
...
HRP\_Denoised            & 0.712 & +346\%  & 0.33 & 0.111\\
...
\begin{figure}[H]
  ...
  \caption{Risk-return scatter for the full 23-strategy comparison.
...
Marchenko--Pastur denoising and spectral detoning --- the two MACI 2025
future-work items --- and precision-matrix partial correlation all aim
to clean the correlation matrix, and all three cost a little Sharpe in a
BTC-led bull market, where filtering structure means trimming exposure
to the asset that ran. The denoising/detoning pair forms a clean
...
identical to IVP --- an earlier draft's version had degenerated to IVP
(\S\ref{sec:methods-cov}) --- but detoning and partial correlation
remain close in weight-rank space (\S\ref{sec:results-convergence}).
...
\paragraph{Hansen SPA.} Across the 22 candidates versus baseline HRP,
...
multiple testing of 22 candidates.} MVP carries the highest studentised
...
23-strategy backtest with $\mathrm{start}=$ \texttt{2022-01-01}
...
We evaluated 23 constructed portfolio strategies --- thirteen HRP-family variants
(the baseline, eight correlation/covariance constructions, four
