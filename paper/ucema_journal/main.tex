...
the comparison to 25 constructed portfolio strategies: thirteen
...
return-tilted NCO, and an NCO--CRISP hybrid); and three crypto-momentum
strategies. Every comparison runs on a survivorship-bias-corrected,
...
24-candidate search, does not reject the null
($p_{\text{consistent}}=0.41$): no edge survives the multiple-testing
correction. A power analysis explains the gap: the detectable Sharpe
edge scales with concentration, and these allocators' edges sit close to
their detection thresholds. Tilting an allocator toward an
estimated-return objective instead destroys it (return-tilted NCO,
$-97\%$ total return), an illustration of the estimation-error
amplification that motivates risk-based allocation. A passive 100\% BTC
...
Iterative Shrinkage Portfolio; \citealp{wuebben2026crisp}). CRISP solves
...
the paper's two best ideas, and reduces exactly to NCO at $\gamma=1$.
...
\textbf{CRISP}           & \textbf{0.989} & \textbf{+1693\%} & 0.30 & \textbf{0.012}\\
\textbf{NCO\_CRISP}      & \textbf{0.977} & \textbf{+1613\%} & 0.45 & \textbf{0.011}\\
...
for the 24-candidate search; the Hansen SPA, which does, is reported in
...
\begin{figure}[H]
  ...
  \caption{Risk-return scatter for the full 25-strategy comparison.
...
allocation is designed to avoid. The hybrid is itself informative ---
it lands \emph{between} its two parents (NCO 0.933, CRISP 0.989), so
the correlation-shrinkage and the hierarchical nesting act as
substitutes, not complements: the shrinkage carries the edge, the
hierarchy adds nothing on top of it. Changing the clustering step
...
hybrid ($+0.239$, $p=0.011$). MVP's larger raw edge ($+0.314$) falls
...
\paragraph{Hansen SPA.} Across the 24 candidates versus baseline HRP,
...
multiple testing of 24 candidates} --- and this is the decisive test:
...
inflated by the 24-candidate search, sits above all of them. CRISP's
...
$+0.31$ and $+0.19$ edges, though economically large, fall short. The
more moderate CRISP and NCO\_CRISP ($N\approx7$--$9$) have thresholds
near $\approx0.28$, which is why their $\approx+0.24$ edges \emph{do}
clear pairwise significance while MVP's larger edge does not.
...
25-strategy backtest with $\mathrm{start}=$ \texttt{2022-01-01}
...
(Ledoit--Wolf $p=0.017$ and $0.022$). Whatever these allocators are
...
leaves the effect strongly suggested but not established.
...
The simulation optimism and the live-data failure together delimit when
signal-aware hierarchical allocation is worth attempting.
...
($p_{\text{consistent}}=0.41$) does not reject. This is not a
...
HRP?''; the SPA asks ``does \emph{the best of 24 searched strategies}
beat HRP, given that we searched 24?'', and inflates the critical
...
We evaluated 25 constructed portfolio strategies (thirteen HRP-family
...
return-tilted NCO collapses to $-97\%$. CRISP and NCO--CRISP even beat
HRP at the pairwise level in both the full and post-COVID windows
(Ledoit--Wolf $p\approx0.01$). But the Hansen SPA, correcting for the
24-candidate search, still does not reject the null
($p_{\text{consistent}}=0.41$): no edge survives the multiple-testing
