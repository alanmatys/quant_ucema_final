...
25-candidate search, does not reject the null
...
peak-to-trough drawdown. ``LW $p$'' is the studentized Ledoit--Wolf
...
for the 25-candidate search; the Hansen SPA, which does, is reported in
...
\paragraph{Hansen SPA.} Across the 25 candidates versus baseline HRP,
...
multiple testing of 25 candidates}, and this is the decisive test:
...
kurtosis of the return series, and the sample length. Treating the 25
...
and NCO ($0.94$) fall just short, and baseline HRP sits at $0.82$.
Counting the 547-cell hyperparameter sweep as part of the search
($N\approx572$) raises the hurdle to $\mathrm{SR}_0=0.57$ and pushes
every strategy below the bar, MVP included ($\mathrm{DSR}=0.91$). The
...
($p_{\text{lower}}=0.73$, $p_{\text{upper}}=0.91$). Although 21 of 547
...
to post-COVID. A representative 17 of the 25 strategies are shown; the
...
\item PIT universe is ranked by Binance quote volume, not market cap.
    ...
    robustness check.
\item Cross-platform reproducibility of the trained encoders. When the
    ...
    HRP\textsubscript{Contrastive} and HRP\textsubscript{TS2Vec}.
    Neural-network training is only conditionally reproducible:
    multi-threaded BLAS reductions and platform-specific kernels make it
    bit-identical only within a fixed software stack. We pin the two
    encoders with single-threaded execution and
    \texttt{torch.use\_deterministic\_algorithms}, which makes them
    bit-identical run-to-run, and the reported figures (generated on
    macOS~26, Apple Silicon, with Python~3.11 and PyTorch~2.12) are
    verified by \texttt{scripts/verify\_embedding\_determinism.py}. On a
    different operating system or processor architecture the BLAS
    kernels differ, and the residual floating-point noise can propagate
    through the discrete single-linkage step, moving those two Sharpes
    by up to $\approx 0.02$ and the Ledoit--Wolf $p$-value attached to
    them correspondingly; the headline ranking, with every embedding
    variant at or below baseline HRP, is unaffected.
\end{enumerate}
...
(Ledoit--Wolf $p\approx0.01$). But the Hansen SPA, correcting for the
25-candidate search, still does not reject the null
($p_{\text{consistent}}=0.41$): no edge survives the multiple-testing
