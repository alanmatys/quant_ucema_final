...
never separated a variant from baseline HRP; varying the
\emph{allocation} step did, in both directions (NCO to Sharpe $0.93$,
the return-tilted NCO to $-97\%$). HRP and all its variants share one
allocation rule, inverse-variance recursive bisection, and that rule,
not the dendrogram, is what holds the family together. For a
practitioner the implication is narrow but useful: on this evidence,
effort spent engineering the clustering distance is lower-yield than
effort spent on the allocation objective.
