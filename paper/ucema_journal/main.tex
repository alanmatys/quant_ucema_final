...
\begin{abstract}
\noindent
We extend the MACI 2025 HRP study by addressing its two stated
...
(combined SPA $p_{\text{consistent}}=0.91$). Strategies that alter the
allocation step produce larger dispersion (notably MVP, CRISP,
NCO--CRISP, NCO), with CRISP and NCO--CRISP significant in pairwise
...
Deflated Sharpe results are directionally consistent with that
conclusion.

Post-COVID results indicate strong regime sensitivity: diversified HRP
variants lose 51--64\% of capital while BTC and BTC-concentrating
...
horizon, and search space, allocation choices are more associated with
performance differences than clustering perturbations, but those edges
